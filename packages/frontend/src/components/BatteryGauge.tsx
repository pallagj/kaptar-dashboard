...
interface Props { voltage: number; warnV: number; unit?: string }

export function BatteryGauge({ voltage, warnV, unit = 'V' }: Props) {
  const isPercent = unit === '%'
  const MIN = isPercent ? 0 : 5.0
  const MAX = isPercent ? 100 : 6.5
  const pct = Math.max(0, Math.min(100, ((voltage - MIN) / (MAX - MIN)) * 100))
  const state = voltage < warnV ? 'low' : voltage < warnV + (isPercent ? 5 : 0.4) ? 'warn' : 'ok'
  const color =
    ...
    <div className="card p-4 flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className={`text-2xl font-semibold mt-0.5 ${textColor}`}>
          {isPercent ? `${voltage.toFixed(0)} %` : `${voltage.toFixed(2)} V`}
        </div>
        <div className="mt-2 h-2 rounded-full bg-slate-700 overflow-hidden">
          <div className={`h-full ${color} transition-all`} style={{ width: `${pct}%` }} />
        </div>
        <div className="flex justify-between text-[10px] text-slate-500 mt-1">
          <span>{isPercent ? '0 %' : `${MIN.toFixed(1)} V`}</span>
          <span>{isPercent ? '100 %' : `${MAX.toFixed(1)} V`}</span>
        </div>
      </div>
    </div>
  )
